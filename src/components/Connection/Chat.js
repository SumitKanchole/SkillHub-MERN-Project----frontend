...
//   // Load user ratings from current user data (simulating sessionStorage)
//   useEffect(() => {
//     if (currentUser && currentUser.ratings) {
//       setUserRatings(currentUser.ratings);
//     }
//   }, [currentUser]);

//   // Save rating to user data (in real app, this would save to sessionStorage)
//   const saveRatingToStorage = (userId, rating, comment) => {
...
//     // In your real application, uncomment the line below:
//     // sessionStorage.setItem("current-user", JSON.stringify(updatedUser));
    
//     console.log("Rating saved to user data:", updatedRatings);
//   };
...
//         console.log("📚 Messages set to state:", formattedMessages);
//       } else {
...
//   // End video call
//   const endCall = () => {
//     // Show rating modal after call ends (only if call was active)
...
//     const existingRating = userRatings[chatUser._id];
//     if (existingRating) {
...
//     saveRatingToStorage(chatUser._id, currentRating, ratingComment);
//     toast.success(`You rated ${chatUser.name} ${currentRating} star${currentRating !== 1 ? 's' : ''}!`);
//     closeRatingModal();
//   };
...
//     const stars = [];
//     for (let i = 1; i <= 5; i++) {
//       const isFilled = i <= (isInteractive ? (hoverRating || currentRating) : rating);
//       stars.push(
//         <span
//           key={i}
...
//                   <small>Your rating: {renderStars(existingRating.rating)}</small>
//                 </div>
//               )}
//             </div>
//           </div>

//           {/* Video Call and Rating Controls */}
...
//               style={{
//                 background: existingRating ? '#ffc107' : 'transparent',
//                 color: existingRating ? '#ffe600ff' : '#ffffffff',
...
//         {/* Rating Modal */}
//         {showRatingModal && (
//           <div className="rating-modal-overlay" style={{
//             position: 'fixed',
//             top: 0,
//             left: 0,
//             right: 0,
...
//                 Rate your experience with {chatUser.name}
//               </h4>
...
function ChatPage() {
  ...
  const fetchChatMessages = async (roomId) => {
    try {
      ...
      const response = await axios.get(`http://localhost:3000/room/${roomId}/messages`, {
        withCredentials: true,
